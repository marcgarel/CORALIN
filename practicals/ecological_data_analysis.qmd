...
```{r}
#Chloroplast
chloro=phyloseq::subset_taxa(physeq, Order=="Chloroplast")
...
```{r}
#ble 
ble<- subset_samples(fam_dominant, Station =="Ble")
gamegou<- subset_samples(fam_dominant, Station =="Gamegou")
pierrebenoit<-  subset_samples(fam_dominant, Station =="Pierre-Benoit")
contrebandier<- subset_samples(fam_dominant, Station =="Contrebandier")
...
ggplot(psmelt(ble), 
  ...
  geom_bar(stat = "identity") +
  scale_fill_manual(values = color_family[1:n_family]) +
  ggtitle("Relative Abundance at Family Level of top 25 Family from ble") +
  xlab("Samples") +
  ylab("Relative Abundance") +
  facet_grid(SampleType~Morphotype, scales = "free_x") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), labs = element_text(size = 14, angle = 45))

ggplot(psmelt(gamegou), 
       aes(x = Sample, y = Abundance*100, fill = Family)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = color_family[1:n_family]) +
  ggtitle("Relative Abundance at Family Level of top 25 Family from gamegou") +
  xlab("Samples") +
  ylab("Relative Abundance") +
  facet_grid(SampleType~Morphotype) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), labs = element_text(size = 14, angle = 45))

ggplot(psmelt(pierrebenoit), 
       aes(x = Sample, y = Abundance*100, fill = Family)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = color_family[1:n_family]) +
  ggtitle("Relative Abundance at Family Level of top 25 Family from pierre benoit") +
  xlab("Samples") +
  ylab("Relative Abundance") +
  facet_grid(SampleType~Morphotype) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), labs = element_text(size = 14, angle = 45))
ggplot(psmelt(contrebandier), 
       aes(x = Sample, y = Abundance*100, fill = Family)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = color_family[1:n_family]) +
  ggtitle("Relative Abundance at Family Level of top 25 Family from contre bandier") +
  xlab("Samples") +
  ylab("Relative Abundance") +
  facet_grid(SampleType~Morphotype) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), labs = element_text(size = 14, angle = 45))
...
```{r}
phyloseq::get_sample(ble, i = "Moraxellaceae")
phyloseq::get_sample(ble, i = "Flavobacteriaceae")
phyloseq::get_sample(ble, i = "Pirellulaceae")
```

:::warning
Des OTUs affiliés aux Moraxellaceae (contenant notamment le genre Acinetobacter) et aux Pseudomonadales ont augmenté chez Acropora valida sous stress. De plus, des espèces de Pseudomonas ont augmenté chez le corail sensible Acropora millepora sous double stress.

 Flavobacteriaceae : Cette famille fait partie des bactéries actives pathogènes qui augmentent en abondance sous stress d'acidification et/ou de réchauffement

 Pirellulaceae : Les Pirellulales sont l'un des ordres bactériens abondants (représentant 10,8 % de l'abondance moyenne) identifiés dans les communautés microbiennes des algues corallines encroûtantes. Les phylotypes de Pirellulales sont également parmi les taxons communs et partagés entre plusieurs espèces de CCA connues.

:::

```{r}
write.table(psmelt(ble),
            file = file.path(output_beta, "ble_familytop25_abundance.tsv"),
            quote = FALSE,
            sep = "\t",
            row.names = FALSE)

write.table(psmelt(gamegou),
            file = file.path(output_beta, "gamegou_familytop25_abundance.tsv"),
            quote = FALSE,
            sep = "\t",
            row.names = FALSE)
write.table(psmelt(pierrebenoit),
            file = file.path(output_beta, "pierrebenoit_familytop25_abundance.tsv"),
            quote = FALSE,
            sep = "\t",
            row.names = FALSE)

write.table(psmelt(contrebandier),
            file = file.path(output_beta, "contrebandier_familytop25_abundance.tsv"),
            quote = FALSE,
            sep = "\t",
            row.names = FALSE)
...
```{r}
source("R/beta_diversity.R")
bleAll<- subset_samples(physeq_rar_rel, Station =="Ble")
familyAll<- phyloseq::tax_glom(bleAll, taxrank = "Family")
...
for(i in dist_methods){
  ...
  iDist <- phyloseq::distance(bleAll, method = i)
  # Calculate PCoA ordination
  iMDS <- ordinate(bleAll, "MDS", distance = iDist)
  ## Make plot. Don't carry over previous plot (if error, p will be blank)
  p <- NULL
  # Create plot, store as temp variable, p
  p <- plot_ordination(bleAll, iMDS, color= "SampleType", shape="Treatment") +
    geom_point(size=4) +
    theme_minimal()
...
```{r}
df <- plyr::ldply(plist, function(p) p$data)
head(df)

names(df)[1] <- "distance"

ggplot(df, aes(Axis.1, Axis.2, color = SampleType, shape = date)) +
...
bleabund <- subset_samples(order_abund, Station =="Ble")

 # we first replace the zeros using
 # the Count Zero Multiplicative approach
tmp <- zCompositions::cmultRepl(bleabund@otu_table,
                                method = "CZM",
                                label = 0,
                                z.warning = 1)
...
```{r}
phyloseq::get_sample(ble, i = "Moraxcellacea")
```